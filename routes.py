...
def preview():
	...
	# Get variables
	variable_names = request.form.getlist('var_names[]')
	variable_types = request.form.getlist('var_types[]')
	dropdown_items = request.form.getlist('var_items[]')
	app_variables = []
	for i in range(len(variable_names)):
		variable = {
			'name': variable_names[i],
			'type': variable_types[i],
			'dropdown_items': [item.strip() for item in dropdown_items[i].split(',')] if variable_types[i] == 'dropdown' else []
		}
		app_variables.append(variable)
	...
	session['app_variables'] = app_variables
...
def create_checkout_session():
	# Ensure this route runs within the application context
	stripe_keys = {
...
def download():
	...
	app_theme = session['app_theme']
	app_mode = session['app_mode']
	app_variables = session['app_variables']

	# Zip boilerplate code
	temp_dir = create_boilerplate(app_name, app_tagline, app_description, app_theme, app_mode, app_variables)
